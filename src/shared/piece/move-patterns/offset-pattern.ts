...
import { MovePattern } from "./move-patterns";
 ...
 */
export class OffsetPattern extends MovePattern {
	constructor(
	 ...
	 */
	public getMatches(board: Board, piece: PieceRef): Move[] {
		const moves: Move[] = [];
		...
		for (let i: number = this.minIters; i <= this.maxIters; i++) {
			...
			if (targetX < 0 || targetX >= board.getWidth() || targetY < 0 || targetY >= board.getHeight()) break;

			const targetSqrCoord: SqrCoord = { row: targetX, column: targetY };
			const targetSqr: Square = board.getSquare(targetSqrCoord);
			const targetPiece: PieceRef | undefined = targetSqr.pieceRef;

			// stop if there is a piece and it's of the same color or the piece can't capture
			if (targetPiece !== undefined && (!this.canCapture || targetPiece.color === board.getTurn())) break;
